...
for e in range(2):
    ...
    pre_train_model.fit_generator(datagen.flow(X_train, y_train,batch_size=256),
                                  samples_per_epoch=aug*X_train.shape[0], epochs=1,verbose = 1)


pre_train_string = pre_train_model.to_json()
pre_train_model.save_weights('pre_train_resnet34_cifar_10.h5')
'''
#pre-training: %
#87.50%
#87.84%
...
pre_train_model = Model(input,encode)
pre_train_model.compile(loss = 'mse',optimizer = 'rmsprop')
pre_train_model.load_weights('pre_train_resnet34_cifar_10.h5')
