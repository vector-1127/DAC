...
X = read_all_images('/home/changjianlong/deep-unsupervised-learning/revise1/data/stl10_binary/unlabeled_X.bin')
X = X.astype('float32')
X -= mean_image
X /= 128.
...
for e in range(2):
    y_train = pre_train_model.predict(X,batch_size = 256)
    pre_train_model.fit_generator(datagen.flow(X, y_train,batch_size=256),
                                  samples_per_epoch=aug*X.shape[0], epochs=1,verbose = 1)

del X
pre_train_string = pre_train_model.to_json()
pre_train_model.save_weights('pre_train_resnet34_stl_10.h5')
'''
#pre-training: %
#87.50%
#87.84%
...
pre_train_model = Model(input,encode)
pre_train_model.compile(loss = 'mse',optimizer = 'rmsprop')
pre_train_model.load_weights('pre_train_resnet34_stl_10.h5')
